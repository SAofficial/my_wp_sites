...
<link rel='stylesheet' type='http://dev1.onlinetestingserver.com/clotho-bros-site/css/datatables.min.css'/>

<script type='text/javascript' src='http://dev1.onlinetestingserver.com/clotho-bros-site/js/datatable-basic.js'></script>
<script type='text/javascript' src='http://dev1.onlinetestingserver.com/clotho-bros-site/js/datatables.min.js'></script>
...
jQuery(document).ready(function(){
  jQuery('#menuBtn').click(function(){
    jQuery('.phone_menu_body').css('display','block');
  });
  jQuery('a#close_btn').click(function(){
    jQuery('.phone_menu_body').css('display','none');
  });
});

jQuery(document).ready(function(){
	jQuery("tbody tr td i").click(function(){
		jQuery(this).next().toggle('show')
		jQuery("tbody tr").addClass("tabeRow");
	});
});

	
jQuery(function() {
    jQuery("div.star-rating > s").on("click", function(e) {
    jQuery(this).closest('div').find('.active').removeClass('active');
    jQuery(e.target).parentsUntil("div").addClass('active');
    jQuery(e.target).addClass('active');  // the element user has clicked on
        var numStars = $(e.target).parentsUntil("div").length+1;
        jQuery('.show-result').text(numStars + (numStars == 1 ? " star" : " stars!"));
    });
});	
</script>
